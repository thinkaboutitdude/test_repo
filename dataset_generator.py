...
def generate_envs(config: UCBConfig):
    ...
    for i in range(config.num_eval_envs):
        env = gym.make(config.env_name, arms_mean=eval_dists['all_new'][i], num_arms=config.num_arms)
        eval_envs_uniform.append(env)
    return (train_envs, eval_envs, eval_envs_uniform)
...
def main(config: UCBConfig):
    ...
    train_envs, _, _ = generate_envs(config)
